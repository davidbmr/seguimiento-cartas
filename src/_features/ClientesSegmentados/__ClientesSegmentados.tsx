...
export const ClientesSegmentados = () => {
	...
	useEffect(() => {
		const fetchSegmentacion = async () => {
			try {
				...
				setDataSegmentado(data.data);
				setPage(data.page - 1);
				setRowsPerPage(data.perPage);
				setTotalItems(data.totalItems);
			} catch (error) {
				console.error("Error al obtener la segmentación:", error);
			}
		};
	...
	const headerGroup = (
		<ColumnGroup>
			<Row>
				<Column header="" colSpan={32} style={{ width: "px" }} />

				<Column header="TC" colSpan={5} />
				<Column header="Consumo" colSpan={2} />
				<Column header="Bisa Hogar" colSpan={5} />
				<Column header="Bisa Auto" colSpan={5} />
			</Row>

			<Row>
				<Column header="" colSpan={9} style={{ width: "px" }} />
				<Column header="Saldos Actuales Clientes" colSpan={4} />
				...
				<Column header="Con Calificación de Riesgo" colSpan={2} />
				<Column header="" colSpan={3} />
				<Column header="Tarjeta de Crédito" colSpan={5} />
				<Column header="Crédito Consumo" colSpan={2} />
				<Column header="Hasta 2da Vivienda" colSpan={4} />
				<Column header="Hasta 3ra Vivienda" colSpan={1} />
				<Column header="Crédito de Auto" colSpan={5} />
			</Row>
			<Row>{columns && columns.map((item: any) => <Column header={item.nombre} />)}</Row>
		</ColumnGroup>
	);

	return (
		<MainContentStructure>
			<HeaderDataTable isSearch placeholderText="Buscar por No. Cliente" />
			<br />
			<DataTable
